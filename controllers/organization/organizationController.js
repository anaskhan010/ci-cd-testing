...
const {sendSubjectCreationEmailToSiteManager} = require('../../services/emailService.js')
...
The Research Hero Team`;
      try {
        await sendEmail(email, emailSubject, first_name, last_name);
      } catch (emailError) {
      ...
      // Sending subject creation email to Site manager 
      try {
        const subject = newData;
        console.log("Sending subject creation email to Site manager")
        await sendSubjectCreationEmailToSiteManager(subject)
      } catch (emailError) {
        console.error("Error in Sending subject creation email to Site manager : ", emailError)
      }
...
function isValidStatusTransition(oldStatus, newStatus) {
  ...
  const validTransitions = {
    ...
    "Screen Failed": ["Screened"],
    "Early Termination": ["Completed"],
    Dropped: ["Completed"],
    Completed: ["Randomized"], // Can only go back to Randomized, not to problem statuses
    "Withdrew Consent": ["Screened", "Randomized"],
    "Lost to Follow up": ["Randomized", "Completed"],
    "Safety Follow-up": [
      "Completed",
      "Early Termination",
