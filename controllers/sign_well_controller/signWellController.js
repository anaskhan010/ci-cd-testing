...
const createDocumentForSigning = async (req, res) => {
  ...
  try {
    ...
    if (filled_by === "Investigator") {
      ...
      const scale_name = await signModel.getScaleNameById(scale_id);

      console.log("Scale name retrieved:=====================", scale_name);

      // Log additional details about the scale name for debugging
      if (scale_name) {
        console.log("Scale name details:", {
          original: scale_name,
          length: scale_name.length,
          hasSpecialChars: /[àáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿĀāĂăĄąĆćĈĉĊċČčĎďĐđĒēĔĕĖėĘęĚěĜĝĞğĠġĢģĤĥĦħĨĩĪīĬĭĮįİıĲĳĴĵĶķĸĹĺĻļĽľĿŀŁłŃńŅņŇňŉŊŋŌōŎŏŐőŒœŔŕŖŗŘřŚśŜŝŞşŠšŢţŤťŦŧŨũŪūĬĭĮįİıĲĳĴĵĶķĸĹĺĻļĽľĿŀŁłŃńŅņŇňŉŊŋŌōŎŏŐőŒœŔŕŖŗŘřŚśŜŝŞşŠšŢţŤťŦŧŨũŪūŬŭŮůŰűŲųŴŵŶŷŸŹźŻżŽž]/.test(scale_name)
        });
      } else {
        console.log("Warning: Scale name is null or undefined for scale_id:", scale_id);
      }

      

      // Send the actual passcode via Paubox with enhanced error handling
      try {
        console.log("Attempting to send passcode email with scale name:", scale_name);
      ...
      // Send the actual passcode via Paubox (for Subject case, we can also include scale name)
      try {
        // Get scale name for subject emails too
        const subject_scale_name = await signModel.getScaleNameById(scale_id);
        console.log("Attempting to send passcode email to subject with scale name:", subject_scale_name);
        ...
        console.error("Subject email error details:", {
          recipientEmail: subjectEmail,
          errorMessage: error.message
        });
        // If Paubox email fails, revert to sending passcode through SignWell
...
async function convertFileToBase64(file_path) {
  try {
    // Normalize file path to handle different OS formats
    ...
    console.log(`File not found or can't be read: ${file_path}`);
    return null

  }
}

const updateDocumentForSigning = async (req, res) => {

  
   
  const { s_id :  signature_id } = req.body;


  const {user_id: userId, scale_id, day_id, investigatorId, filled_by, file_path: filePathFromDatabase} = await signModel.getScaleDetails(signature_id);

  // write code, if there is no record for that signature then write that there is no such scale record available.
  ...
  try {
    ...
    if (filePathFromDatabase.includes("signed/") ){
      file_original_name = filePathFromDatabase.split("signed/")[1]
    }
    else if (filePathFromDatabase.includes("signed\\")){
      file_original_name = filePathFromDatabase.split("signed\\")[1]
    }


    const fileBase64 =  await convertFileToBase64(filePathFromDatabase);

    // if file not found the display error message
    if (!fileBase64){
      return res.status(400).json({ message: `File not found or can't be read: ${file_original_name}` });
    }
    ...
    ("Signee1 data:", JSON.stringify(Signee1, null, 2));
    ("Signee2 data:", JSON.stringify(Signee2, null, 2));

    // decrypt the first and last names for investigator
    const { first_name, last_name, email: investigatorEmail } = Signee2 || {};

    // Add additional logging to debug the decryption process
    ("Investigator first_name before decryption:", first_name);
    ("Investigator last_name before decryption:", last_name);
    ...
    // Add additional logging to debug the decryption process for subject
    ("Subject first_name before decryption:", subjectFirstName);
    ("Subject last_name before decryption:", subjectLastName);
    ...
    ("Subject first_name after decryption:", subjectFirstName);
    ("Subject last_name after decryption:", subjectLastName);
    ...
    if (filled_by === "Investigator") {
      ...
      ("Scale name retrieved:=====================", scale_name);
      ...
      if (scale_name) {
        ...
        ("Warning: Scale name is null or undefined for scale_id:", scale_id);
      }
      ...
      // Send the actual passcode via Paubox with enhanced error handling
      try {
        ("Attempting to send passcode email with scale name:", scale_name);
        await sendPasscodeEmail(
          investigatorEmail || "moe@sentrixmedia.com",
          signee2_passcode,
          investigatorFirstName,
        ...
        ("Passcode email sent to investigator successfully");
      } catch (error) {
        console.error("Error sending passcode email to investigator:", error);
        console.error("Email error details:", {
          scaleName: scale_name,
      ...
      try {
        // Get scale name for subject emails too
        const subject_scale_name = await signModel.getScaleNameById(scale_id);
        ("Attempting to send passcode email to subject with scale name:", subject_scale_name);
        ...
        ("Passcode email sent to subject successfully");
      } catch (error) {
        console.error("Error sending passcode email to subject:", error);
        console.error("Subject email error details:", {
          recipientEmail: subjectEmail,
    ...
    // Step 4: Send the document data to SignWell API
    ("Sending document data to SignWell with passcode notification");
    const postResponse = await signwell.postApiV1Documents(documentData);

    ("SignWell API Response received successfully");
    ...
    // Step 5: Fetch the document details using SignWell API
    const getResponse = await signwell.getApiV1DocumentsId({ id: documentId });
    // ("Document details fetched:", getResponse.data);
    ...
    // Step 7: Save the document information in your database
    const result = await signModel.updateDocumentForSigning(
      userId,
      scale_id,
      day_id,
      investigatorId,
      ...
      signee1_passcode,
      signee2_passcode,
       undefined,         // <-- This will let `status` use its default "Enable",
      signature_id
    );
    ...
    console.error("Error in updateDocumentForSigning:", error);
